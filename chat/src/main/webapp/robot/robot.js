(function(win, options, undefined){
	...
	var hideKefu = false										//是否隐藏人工客服
		, hideList = true 										//是否隐藏功能列表
		, cssfix = 'smartnlp-robot-'							//css类名前缀
		, apiUrl = '127.0.0.1:9001/qa'							//机器人后台接口地址
	...
	var _jsonp = function(options, time, type){
		...
		console.log(o.url + (o.url.indexOf('?') > 0 ? "&" : "?") + argument)
	};
	...
	var _generateHtml = function(source){
		...
		if(!!_con.showCopyright){
			html.push('<a class="'+ cssfix +'clink" href="http://smartnlp.cn" target="_blank">Powered by 奇智机器人</a>');
		};
	...
	Robot.prototype = {
			...
			var connKefu = function(){
				if(_isConnChat === -1){
									...
									if (!error) {
										...
										con.call('methodTheRequiresBeingLoggedIn', function () {
											...
											con.call("createDirectMessage", kfName, function(error,result){
												...
												params = {'connection': con,'message': messages,'rid': rid,'kfInfo':kfInfo};
					...
					sendImgMessage = function(params, text_msg, img_url){
						if (params !== null) {
							params.connection.call('sendImgMessage', {'rid': params.rid, 'msg': text_msg, 'img': img_url});
							return;
						}
					};
		...
		, 'upload': function($ele, settings){
			...
			var options = $.extend({  
				...
				successFn: function(){},							 //上传成功后执行的方法 uploadSuccess(response, statusText, xhr, $this)  
		...
		, 'send': function(text){
			...
			_jsonp({
				//机器人后台部署地址
				'url': 'http://127.0.0.1:9001/qa'
				, 'data' : {'question': encodeURIComponent(text)}
	...
	, 'welcome': 'Hi，我是$name'
	//人工客服欢迎语
	, 'kefuWelcome': '多一些润滑，少一些摩擦： 统一润滑油<br/>您好，$name 为您服务!'
	//机器人昵称
	, 'nick': '智能助手-JARVIS'
	//消息发送后文本框是否自动失去焦点
	//, autoblur: false
	//新窗口打开新页面的URL,手机版有效
	, 'newurl': '?mobile'
	, 'logo': 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1561959143365&di=e606680078ceb4373ab68a3ef354ec84&imgtype=0&src=http%3A%2F%2Fimage.thepaper.cn%2Fwww%2Fimage%2F7%2F87%2F528.jpg'
